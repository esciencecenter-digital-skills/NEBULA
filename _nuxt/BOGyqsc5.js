...
`,l?'<pre><code class="'+this.options.langPrefix+V(l)+'">'+(e?n:V(n,!0))+`</code></pre>
...
`),w.markdown=y[0]&&typeof y[0].getAttribute("data-markdown")=="string"),s.postMessage(JSON.stringify(w),"*")}function f(d){if(function(k){try{return window.location.origin===k.source.location.origin}catch{return!1}}(d))try{let k=JSON.parse(d.data);k&&k.namespace==="reveal-notes"&&k.type==="connected"?(clearInterval(R),g()):k&&k.namespace==="reveal-notes"&&k.type==="call"&&function(y,A,w){let S=r[y].apply(r,A);s.postMessage(JSON.stringify({namespace:"reveal-notes",type:"return",result:S,callId:w}),"*")}(k.methodName,k.arguments,k.callId)}catch{}}function g(){r.on("slidechanged",a),r.on("fragmentshown",a),r.on("fragmenthidden",a),r.on("overviewhidden",a),r.on("overviewshown",a),r.on("paused",a),r.on("resumed",a),a()}return{id:"notes",init:function(d){r=d,/receiver/i.test(window.location.search)||(window.location.search.match(/(\?|\&)notes/gi)!==null?i():window.addEventListener("message",k=>{if(!s&&typeof k.data=="string"){let A;try{A=JSON.parse(k.data)}catch{}A&&A.namespace==="reveal-notes"&&A.type==="heartbeat"&&(y=k.source,s&&!s.closed?s.focus():(s=y,window.addEventListener("message",f),g()))}var y}),r.addKeyBinding({keyCode:83,key:"S",description:"Speaker notes view"},function(){i()}))},open:i}}})})(Ft);var ye=Ft.exports;const ve=ie(ye);/*!
 ...
 */const _e=()=>{let x,n,t,e,l,h,c;function p(){n=document.createElement("div"),n.classList.add("searchbox"),n.style.position="absolute",n.style.top="10px",n.style.right="10px",n.style.zIndex=10,n.innerHTML=`<input type="search" class="searchinput" placeholder="Search..." style="vertical-align: top;"/>
  ...
  `,n.prepend(e)}const Te={class:"flex relative box-border h-full reveal bg-eScienceWhite z-10"},ze={class:"slides flex h-full w-full"},Ae={"data-markdown":"","data-separator":"^\\r?\\n===\\r?\\n$","data-separator-vertical":"^\\r?\\n==\\r?\\n$","data-separator-notes":"^Note:"},Ee={props:{slidescontent:{type:String,default:"Missing Document"}},methods:{stripFrontmatter(x){return x.replace(/^---$.*?^---$/ms,"")}}},Re=Zt({...Ee,__name:"Slides",setup(x){return Kt(()=>{console.log("Slides Mounted"),Rt(()=>import("./B3O8JSaZ.js"),[],import.meta.url).then(n=>{Rt(()=>import("./DN7Rh_EM.js"),[],import.meta.url).then(t=>{new n.default().initialize({controls:!0,progress:!0,center:!0,hash:!0,transition:"none",embedded:!0,showNotes:!0,plugins:[we,t.default,ve,Se,_e]})})})}),(n,t)=>(ft(),yt("div",Te,[vt("div",ze,[vt("section",Ae,Xt(n.stripFrontmatter(x.slidescontent)),1)])]))}}),Ie={key:0,class:"overflow-hidden h-full border-8 rounded-xl border-eScienceWhite z-10"},Le={key:1,class:"flex justify-center items-center"},Ce={class:"flex m-4 my-10 w-2/3 bg-eScienceWhite justify-center py-8 px-12"},He=Zt({__name:"[chapter]",async setup(x){let n,t;const e=Dt(),l=Yt();return[n,t]=Gt(()=>ne("/modules/"+l.params.module+"/").sort({order:1,$numeric:!0}).find()),n=await n,t(),""+e.public.baseURL,(h,c)=>{const p=Re,b=re,_=le;return ft(),te(_,null,{default:ee(({doc:u})=>[u.type==="slides"?(ft(),yt("div",Ie,[It(p,{slidescontent:u.plainText},null,8,["slidescontent"])])):(ft(),yt("div",Le,[vt("div",Ce,[It(b,{value:u,class:"font-body prose-lg max-w-4xl"},null,8,["value"])])]))]),_:1})}}});export{He as default};
